...
class RunnerTests(unittest.TestCase):
    def test_full_mock_matrix_and_evidence(self):
        scenarios = build_mock_scenarios()
        self.assertEqual(len(scenarios), 128)
        self.assertEqual(len({case.name for case in scenarios}), 128)
        with tempfile.TemporaryDirectory() as folder:
            report = AutomatedTestRunner().run(scenarios, folder)
            self.assertEqual((report["passed"], report["failed"]), (128, 0))
            saved = json.loads((Path(report["evidence_dir"]) / "results.json").read_text())
            self.assertEqual(saved, report)
            self.assertTrue(all(row["mode"] == "mock" for row in saved["results"]))
            self.assertEqual(len((Path(report["evidence_dir"]) / "results.log").read_text().splitlines()), 129)
